...
- **链式过滤**：多个过滤器顺序叠加（字符串或正则模式），每层缓存中间结果；`x` 键切换模式，过滤栏显示每个过滤器的匹配行数及颜色指示圆点（⬤ 启用 / ○ 禁用，均显示过滤器颜色，选中时整体高亮）
...
**v0.9.6 — 搜索 UX 改进 + 过滤器圆点优化**（288 个测试，全部通过）
...
- v0.9.6：搜索词反色、ESC/Tab 清除搜索、过滤器圆点优化（⬤/紧贴/选中同步高亮）、水平滚动限制、搜索正则模式切换（Tab）、鼠标滚轮每次 1 行 ✓
