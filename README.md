...
- **链式过滤**：多个过滤器顺序叠加（字符串或正则模式），每层缓存中间结果；`x` 键切换模式，过滤栏显示每个过滤器的匹配行数及颜色指示圆点（● 启用 / ○ 禁用，均显示过滤器颜色）
...
- v0.9.5：行号右对齐（内容列固定）、禁用过滤器彩色圆点、窗格 6:4 比例、ESC 随时中断过滤、启动不自动打开上次文件、Windows 版本资源 ✓
