...
- **链式过滤**：多个过滤器顺序叠加（字符串或正则模式），每层缓存中间结果；编辑过滤器时 `Tab` 切换正则/字符串模式（与搜索逻辑统一），过滤栏显示每个过滤器的匹配行数及颜色指示圆点（⬤ 启用 / ○ 禁用，均显示过滤器颜色，选中时整体高亮）
- **水平滚动**：`←`/`→` 键水平平移当前窗格内容，查看超出终端宽度的行
- **鼠标支持**：滚轮上下滚动悬停窗格（每次 1 行）；左键单击切换焦点（原始区 / 过滤区）
...
- **搜索**：焦点感知关键字搜索（原始区或过滤区），输入时 `Tab` 切换正则/字符串模式，`n`/`p` 前后跳转，匹配词反色显示；`Esc` 或 `Tab` 切换窗格时清除搜索；底部状态行持续显示当前搜索词及结果计数
...
| `Space` | 启用 / 禁用当前过滤器 |
| `[` / `]` | 上 / 下选择过滤器 |
| `/` | 搜索（输入时 `Tab` 切换正则/字符串模式，输入行实时显示合法性圆点） |
| `n` / `p` | 跳转到下 / 上一个搜索结果 |
...
**v0.9.7 — 交互逻辑统一 + 代码重构**（292 个测试，全部通过）
...
- v0.9.7：正则切换统一为 Tab（过滤器/搜索均一致）、输入行合法性圆点统一、s 键切换静态/实时模式、代码坏味道清理（clearSearch 抽取、魔法数字集中配置） ✓
