...
- v0.9.4：退出确认弹窗、过滤器颜色圆点、搜索词状态栏、行号默认开启、原始区不染色、折叠行颜色保留、过滤超时确认 ✓
- v0.9.5：行号右对齐（内容列固定）、禁用过滤器彩色圆点、窗格 6:4 比例、ESC 随时中断过滤、启动不自动打开上次文件、Windows 版本资源 ✓
- v0.9.6：搜索词反色、ESC/Tab 清除搜索、搜索正则模式切换（Tab）、过滤器圆点优化（⬤/紧贴/选中同步）、水平滚动限制、鼠标滚轮每次1行 ✓
